...
import { GITHUB_ACTIONS, githubReducer, initialState, type GithubModalType } from './types';
import type {
...
export const useGithubActions = () => {
  ...
  const { data: repos = [], isLoading: isReposLoading } = trpc.githubRepo.listRepos.useQuery();
  const { data: projects = [], isLoading: isProjectsLoading } = trpc.projects.list.useQuery({});
  const { data: stats = null, isLoading: isStatsLoading } = trpc.githubStats.getStats.useQuery();
