...
export async function invokeAction<
  ...
  TInput extends TService[TMethod] extends (input: infer I) => unknown
    ? I
    : never,
  TReturn extends TService[TMethod] extends (input: never) => infer R
    ? R
    : never,
>(
  ...
  try {
    ...
    if (typeof action !== "function") {
      throw new Error(
        `[Service Error] Method ${String(method)} not found on ${ServiceClass.name}`,
      );
    }

    const callableAction = action as unknown as (
      input: TInput,
